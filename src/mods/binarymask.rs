pub fn frommask(args: &[String]) {
    ...
    let a = format!("{:b}", mask).chars().map(|c| c.to_digit(10).unwrap()).collect::<Vec<u32>>();
...
pub fn tomask(args: &[String]) {
    ...
    /* ie 0,1,0 */
    let a = args[0].split(",").map(|s| s.parse::<u32>().unwrap()).collect::<Vec<u32>>();
    let mask = u32::from_str_radix(&a.iter().map(|&i| i.to_string()).collect::<String>(), 2).unwrap();
    println!("{:?}", mask);
}